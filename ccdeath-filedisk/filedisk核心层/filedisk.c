...
#include <ntddcdrm.h>
#include <ntverp.h>
//注释部分made by ccdeath，对参考其他人表示感谢。
//
...
HANDLE dir_handle;
//你必然要在磁盘设备对象上保留一些私人信息，因此不能把设备扩展大小设置为0。
//你应该定义设备扩展的数据结构。
...
VOID
FileDiskUnload (//卸载驱动，删除设备，关闭打开的文件等等。
    IN PDRIVER_OBJECT DriverObject
    )
{
    PDEVICE_OBJECT device_object;
...
{
//PAGED_CODE()是一个调试用的宏。如果一个函数被定义在可分页交换的段中，那么它执行时的中断级别必须满足一定的要求。
    PAGED_CODE();
//下面是经典模式，没什么好说的
